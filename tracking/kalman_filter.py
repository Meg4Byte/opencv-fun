...
kalman.measurementNoiseCov = np.array([[1e-3, 0],
                                       [0, 1e-3]], np.float32)
    ...
    if len(cnts) > 0:
        ...
        if cv2.contourArea(c) > 0:
            ...
            if radius > 5:
                ...
                if center[1] <= args["roi_y"] and pts and pts[-1] and pts[-1][1] > args["roi_y"]:
                ...
                # Draw the predicted state from the Kalman filter
                cv2.circle(roi, (int(prediction[0]), int(prediction[1])), 5, (255, 0, 0), -1)
